...
function disableChat(is_expired) {
    ...
    if (has_cheap_talk)
        setDisabled(inputCheapTalk, buttonCheapTalk, placeholder)

    if (has_chat_gpt)
        setDisabled(inputGPT, buttonGPT, placeholder)
}
...
test('function sendChatMessage ChatGPT', () => {
    ...
    const fakeChatBoxGPT = createChatBoxElement("div", "chatbox-gpt", "Ancien: Message<br>", 0, 1000);
    ...
    expect(input.value).toBe("");
    expect(input.focused).toBe(true);
    expect(fakeChatBoxGPT.innerHTML).toBe("Ancien: Message<br>Toto: Ceci est une valeur de test !<br>");
...
test('function handleChatGPTReply', () => {
    ...
    const fakeChatBoxGPT = createChatBoxElement("div", "chatbox-gpt", innerHTML, 0, 500);

    handleChatGPTReply(data);

    expect(fakeChatBoxGPT.innerHTML).toBe("Ceci est une question.<br>Robot: Ceci est une réponse.<br>");
...
test('function handleNewMessage', () => {
    ...
    const fakeChatHistory = createChatBoxElement("div", "chat_history", innerHTML, 0, 200);

    handleNewMessage(data);

    expect(fakeChatHistory.innerHTML).toBe("Ceci est un ancien message...Ceci est un nouveau message !");
    expect(fakeChatHistory.scrollTop).toBe(200);
});

test('function handleTypingIndicator typing', () => {
    const data = { other_player_typing: true };
    const fakeTypingIndicator = createElement("div", "typing_indicator", "test-class d-none toto");
...
test('function handleTypingIndicator not typing', () => {
    ...
    const fakeTypingIndicator = createElement("div", "typing_indicator", "test-class toto");
...
test('function handleSentStatus', () => {
    ...
    const fakeWaitingMessage = createElement("div", "waiting_message", "toto d-none");
...
test('function handleReceivedStatus', () => {
    ...
    const fakeReceivedAmountDisplay = createElement("div", "received_amount_display", "titi d-none");
